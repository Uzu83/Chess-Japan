...
| プラン | 上限 | 状態 |
|---|---|---|
| 無料(匿名) | **50解説/日/IP**（`RATE_PER_DAY=50`・15/分は維持） | **決定済み**。secrets 変更のみで適用 |
| 無料アカウント | 100/日（案） | Phase 2C（Supabase Auth）とセットで実装 |
| 有料 ¥400〜500/月 | 1000/日（案）。heavy user 原価 ~250円/月 → 粗利確保 | Phase 2C+Stripe。課金設定は手動承認領域 |
