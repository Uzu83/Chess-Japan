...
**結果**（相対 Elo 差。0/16 等の飽和ペアは下限）:

| ペア | 勝敗(下位-上位) | 下位の得点率 | Elo差 |
|---|---|---|---|
| easy vs normal | 0-16 | 0% | ≥ ~500（飽和） |
| normal vs hard | 1-15 | 6% | **≈ 470** |
| hard vs max | 1-15 | 6% | ≈ 470（max は測定用に 500k ノードへ軽量化＝実アプリの max はさらに強く gap は下限） |
| easy vs hard | 0-16 | 0% | ≥ ~800（飽和） |

**結論**: プリセットは **順序どおり分離**（upset なし）し、各段 ~470-600 Elo で実質的に離れている。かつ現行の表示 elo（800/1400/1900/2800・gap 600/500/900）は実測と整合する（normal→hard は表示500 vs 実測470でほぼ一致、gap 形状=中央が最小も一致）。よって **暫定値は実測で妥当と検証**され、「暫定→検証済み」へ格上げ（値の変更は不要と判断＝飽和データに恣意的な新値を当てるのは過剰適合）。絶対 Elo は将棋で基準が曖昧（ウォーズ/81dojo/floodgate で桁違い）なので「~」目安表記は維持。

**再現性**: プリセットの skill/nodes を変えたら `node scripts/measure-shogi-elo.mjs [games]` を再実行して回帰確認する（ハーネスをコミット済み）。
