...
- 🔄 **Phase 2C-1 実装中（2026-07-07・Codex ゲート①合意済み）**: Google OAuth 単独(redirect/PKCE)・profiles(owner スコープ RLS + 列 GRANT で rating は RPC 専用)・初期Elo オンボーディング・ローカルレート移行・`VITE_AUTH_ENABLED` フラグで段階公開。**2C-1 の意図的制約**: AI戦結果→クラウド反映は 2C-2（PlayView は無改変。表示クラウドレートはアカウントメニュー内のみ = 「表示はクラウド・更新はローカル」の嘘 UX を避けた）。migration 0004/0005 は作成済み・**本番適用はオーナー GO 待ち**。
