...
**コスト防衛（不変条件・非緩和）**: backend は `buildPrompt` に chess/shogi の表記指示分岐（将棋は USI 座標でなく日本語表記 ▲７六歩 で駒種・成り・打ちを明示）を足しただけ。rate/validate/Turnstile/キャッシュには一切触れていない（validate.ts は既に shogi 受理済み）。`deno check` 緑。**Edge Function v10 として本番デプロイ・E2E 確認済み（2026-07-08）**。

**本番 E2E 結果（2026-07-08・Playwright）**: 本番で KIF 読込 → やねうら王実解析 4/4（503ms）→ Turnstile 実通過 → Gemini 解説生成まで縦貫通を確認。散文中の生 USI は**ゼロ**（新プロンプト有効）。ただし LLM が手名を**出発地基準で誤命名**する事例あり（正: ▲２二角成 → 出力: ▲８八角成。将棋表記は移動先基準）。**4-2 での改善**: 正しい日本語ラベル（tsshogi の formatMove 出力）を ExplainContext の DATA に同梱して LLM に変換させない（validate.ts 拡張を伴うため Codex ゲート対象）。
...
### Phase 4-2（将棋 AI 対局）— 2026-07-08

**スコープ**: 対局タブに [チェス|将棋] 切替を足し、やねうら王と対局 → 終局（詰み/千日手/連続王手/投了）→ 履歴保存 → 「この対局を振り返る」で 4-1 のレビューへ接続。4-1 で未配線だった `yaneuraou.chooseMove` を配線。**チェス経路は 1 挙動も変えず、将棋一式はチェス利用者のメインチャンクに 1 バイトも載せない**（build 実測で確認済み）。

**構造（Codex ゲート① 合意・修正 5 点を織り込み）**:
- **共通シェル + kind 別セッション（修正 #1）**: `PlayView.tsx` に kind 切替を追加するだけ。chess は既存 JSX を hidden で常時マウント（DOM を破棄せず挙動を完全維持）、shogi は `React.lazy(ShogiPlaySession)` を Suspense で描画。PlayView のリファクタはしない（回帰ゼロ優先）。
- **`ShogiPlaySession.tsx`（新規・lazy 到達専用）**: PlayView の確立パターン（snapshot in state / mutable in ref・turnToken キャンセル・終局後拒否・0手対局は保存スキップ・レート戦/待った降格）を**踏襲した独立実装**。単一抽象で押し切らない（質問1回答）。
- **`shogiPlayGame.ts`（新規・tsshogi ベースの可変コントローラ）**: move/drop/applyUsi/undo/resign/outcome/exportKif/snapshot。終局判定は tsshogi の `record.repetition`/`record.perpetualCheck` に委ねる（連続王手の千日手＝王手継続側の負け。node 実測で構築した SFEN/手順で検証）。打ち歩詰め・二歩・行き所のない駒は isValidMove が弾く。**千日手の反復局面が ply0（初期局面）始まりだと perpetualCheck 判定が START ノードで崩れる地雷**があり、テストは前置き1手を入れて回避（コメント明記）。
- **`ShogiPlayBoard.tsx`（新規・lazy 到達専用）**: shogiground の操作盤。movable.dests で着手、駒台タップ→ droppable.dests で打ち。**内蔵の成りダイアログは全無効化**し、PlayBoard(チェス) と同じモーダルイディオムの「成る/不成」ピッカーへ統一（強制成りは core が自動）。
- **`playController.ts`（新規・型定義のみ・実装なし＝修正 #4）**: chess/shogi 可変コントローラが満たす共通の面をドキュメント兼コンパイル契約として言語化。今は実装を寄せない（回帰リスク回避）が寄せる余地を型で残す。
- **storage（修正 #3）**: `PlayedGame.game?:'chess'|'shogi'` を **optional** 追加。読み出しは `playedGameKind()`＝`game ?? 'chess'` で既存履歴を落とさず移行（デシリアライザは値を注入せず round-trip の純粋性を保つ＝既存テスト不変）。将棋は KIF を既存 `pgn` フィールドに格納し**フィールド名の負債を許容**（リネームは isPlayedGame 互換を壊すため・コメント明記）。レートは**将棋別枠 `cj:rating:shogi`**（質問3回答）。`loadRatingFor/saveRatingFor(kind)` を追加、chess は既存 `loadRating/saveRating` へ委譲し挙動不変。
- **振り返り接続（修正 #2）**: `App.handleReview` / `PlayView.onReview` を `{kind, text}` 受けへ変更（h1・既存テストのアサーション不変）。`ReviewView` の `initialPgn` → `initialRecord?:{kind,text}` へ移行（呼び出し元を全更新）。将棋振り返りは `kindRef.current='shogi'` を同期で立ててから `loadShogi()`（4-1 の race ガードと整合）。
- **COI 判定は UI 側で先に（修正 #5）**: `crossOriginIsolated===false`（Safari 等）ならセットアップで「エンジン非対応・対局不可」を表示し開始ボタンを封じる。factory の mock フォールバック（MockShogiEngine は指せない）には委ねない。

**難度プリセット（暫定・要実測）**: SkillLevel 2/8/14/20（目安 ~800/~1400/~1900/~2800）。**やねうら王の SkillLevel×NodesLimit の実効 Elo は未実測**で、chess プリセットに倣った暫定値。将棋の実対局データが取れ次第、elo（レート変動カーブ）と skill/movetime を再調整する。

**スコープ外（やらない）**: 持将棋宣言ボタン / 将棋版「この局面から対局」(4-3) / PlayView チェス側のリファクタ / backend 変更。

**バンドル実測（1バイト不変条件・確認済み）**: メインエントリ `index-*.js` に shogi 実装トークン（createMoveByUSI / formatMove / sg-board / Shogiground / perpetualCheck / ShogiPlayGame / exportKIF / InitialPositionSFEN / ☗）は **0 件**。主要 lazy チャンク: `ShogiPlaySession`≈25.6KB(gz8.3) + shogiground≈45.8KB(gz14.3) + tsshogi(detect)≈68.5KB(gz20.6) + yaneuraou wrapper≈3.8KB（将棋対局に入ったときだけ読込）。メイン側に残るのは factory の動的 import パス文字列・kind リテラルのみ（4-1 と同じ・実装コードではない）。

**検証**: typecheck / lint / format / format:check / test（223 = 4-1 の 199 + shogiPlayGame 17 + storage 移行/将棋レート 7）/ build 全通過。

**✅ 出荷記録（2026-07-08・PR #10 マージ・本番デプロイ済み）**: Codex ゲート②は **blocking 0件**で通過。NTH 採用: AI の `bestmove resign`/不正 USI で無音停止しない（null=AI 投了扱い・applyUsi 失敗=エラー表示。やねうら王は劣勢で実際に resign を返す）。実ブラウザ E2E（dev + 本番）: 対局開始 → ▲７六歩（trusted click）→ AI 応手 → 投了 → 将棋別枠レート -8 → 履歴 → 振り返り接続。チェス側回帰なし（`cj:rating` と `cj:rating:shogi` の分離を localStorage 実値で確認）。

**4-2 バックログ**: ①難度プリセット（Skill 2/8/14/20）の実効 Elo 実測調整 ②ShogiPlaySession のコンポーネントテスト（mock engine で AI先手/stale token/待った降格/0手投了）③tsshogi perpetualCheck は初期局面始まりの反復で判定が崩れる仕様（実戦稀・テストで回避策明記済み）④解説 DATA への日本語手ラベル同梱（4-1 記録参照・validate 拡張 = Codex ゲート対象）。

### Bugfix（将棋盤の見た目バグ・scaleDownPieces）— 2026-07-09（PR #12・本番デプロイ済み）

**症状（ユーザー報告・本番）**: 将棋盤（対局・レビュー両方）で 40 駒が半分サイズ＋translate 半分刻みで盤の左上 1/4 に凝縮。座標も「987654321…」と重なって崩壊。チェス側は無関係。

**根本原因**: shogiground のデフォルトは `scaleDownPieces:true`（公式サンプルは駒スプライトを 2マス幅で用意し scale(0.5) で 1マスに収める前提）。本プロジェクトの `shogiBoard.css` は駒を 1マス幅（width:11.111%）の漢字グリフで描くため、true だと駒 transform が `translate(n*50%) scale(0.5)` になり凝縮する。**Phase 4-1 で CSS は false 前提で書いたがフラグ設定を忘れていた**（4-1・4-2 とも潜伏）。

**なぜスモークで見逃したか（再発防止）**: Phase 4-1/4-2 の検証は棋譜/手番など「状態」だけを見て、`getBoundingClientRect` による「見た目（駒サイズ・配置）」を検証しなかった。**将棋盤の描画は今後 bounding box を実測する**（回帰テスト + コメントで固定）。

**修正**:
- `src/ui/shogigroundConfig.ts`（新設）に両盤共有の視覚不変条件 `SHOGIGROUND_VISUAL_INVARIANTS`（`scaleDownPieces:false` + `coordinates.enabled:false`）と、両盤の config ビルダ（`buildShogiReviewConfig` / `buildShogiPlayConfig`）を集約。`usiToLastDests` / `splitSfen` の両盤重複も解消。
- 対局盤の座標を無効化（自前 CSS に座標配置スタイルがなく、有効だと `position:static` で盤左上に積み上がる。棋譜パネルが日本語表記〈☗７六歩〉を出すので盤上座標は必須でない。閲覧盤と一貫化）。
- 対局盤 config を pure 関数化（event は ref 越しコールバックを引数で受ける）し `ShogiBoard.config.test.ts` で両盤とも設定値を固定（7 tests）。

**レビュー**: Codex ゲート② サイクル1で F001（medium: 回帰テストが閲覧盤だけで対局盤の退行を捕まえられない）を指摘 → 共有モジュール化 + 対局盤 pure 関数化で対応 → サイクル2で findings 空・合意。

**検証**: 230 tests 緑・typecheck/lint（警告0）/format/build 緑・メインチャンクへの shogi ライブラリ混入なし（1バイト不変条件維持）。実ブラウザ（Playwright）で対局盤・レビュー盤とも駒 55px=マス幅・盤いっぱい（span 496/500）・座標要素0・transform `translate(100%刻み) scale(1)` を `getBoundingClientRect` で実測。本番 URL でも同値を再確認。

### Phase 4-3（将棋の局面から対局）— 2026-07-09（PR #14）

**スコープ**: チェス Phase 2B「この局面から対局」の将棋写像。2 入口を解禁。①レビュー画面「▶ この局面から対局」（表示中 SFEN から対局・`COI_ENABLED` のときのみ表示）②対局設定「局面(SFEN)から対局する」折りたたみ（詰将棋・練習・途中局面）。**コアの `ShogiPlayGame(startSfen)` は既に任意 SFEN 開始対応済み**で、主変更は UI 配線 + 純粋バリデーション。設計・裁定・E2E の全文は `docs/progress/phase-4-3-plan.md`。

**構造**:
- `validateStartSfen`（新規 pure 関数・`shogiPlayGame.ts`）: `newBySFEN` 構文検証 + 先手玉/後手玉 各1枚 + 駒種別総数上限（歩18・香桂銀金4・角飛2）。**存在ではなく個数**で弾く（`newBySFEN` は玉なし・重複玉・過剰持ち駒を通すため）。UI から呼ぶが tsshogi 依存は lazy チャンクに閉じる。
- `ShogiPlaySession`: `startGame` に `startSfen` opts（手番側=あなた・強制カジュアル＝チェスと対称）、入口A の `playFrom` effect（nonce ガード・`!COI_ENABLED` で開始封じ・二段バリデーション）、入口B の設定 UI。
- `PlayView`: `playFrom` を kind 分岐（将棋は `switchKind('shogi')` + SFEN を ShogiPlaySession へ prop 伝播、開始は将棋側 effect が担当＝責務分離）。`App`/`ReviewView`: `onPlayFrom` を `(position, kind)` 化。

**Codex ゲート**: ①medium2（F001 0手レビュー矛盾=受入条件を明確化・修正案は却下/F002 玉検証=個数化で採用）②medium1+low1（F001 チェス回帰=`switchKind('chess')` 追加/F002 駒数上限=`exceedsPieceLimits` 追加）。全て採用または反論付き明確化で合意。

**検証**: 243 tests 緑（230 + validateStartSfen/駒数/振り返り接続 13）・typecheck/lint（警告0）/format/build 緑。1バイト不変条件維持（メインエントリに shogi/`validateStartSfen`/`exceedsPieceLimits` 0 件・345KB で 4-2 と同水準）。実ブラウザ E2E（`getBoundingClientRect` 実測）: 入口A/B とも盤 500px・駒55px=マス幅・40駒・`scale(1)`・手番導出正・0手新規、バリデーション拒否表示、F001 チェス復帰、コンソールエラー0。振り返り接続（カスタム開始→exportKif→`fenAt(0)` 一致）は単体で固定。

### 難度Elo実測（Phase 4-2 バックログ① の消化）— 2026-07-10

**背景**: 将棋 AI 対局の難度プリセット（`ShogiPlaySession` の `DIFFICULTIES`。elo=強さ表示＋レート戦の相手レート）は当初チェス流用の暫定値で「要実測」だった。

**方式**: やねうら王(k-p) を **Node ヘッドレスで自己対局**させて相対強度を実測（`scripts/measure-shogi-elo.mjs`）。crossOriginIsolated 不要（Node の worker_threads+SAB で動く）・eval は k-p 埋め込みで外部ファイル不要・1スレッド/定跡off。局面管理と終局判定は tsshogi に委ね（アプリの `shogiPlayGame` と同権威）、先後入替で色相殺、`go nodes` で高速決定的に計測。16局/ペア。

**結果**（相対 Elo 差。0/16 等の飽和ペアは片側下限。16局で信頼区間は広い）:

| ペア | 勝敗(下位-上位) | 下位の得点率 | Elo差（点推定） |
|---|---|---|---|
| easy vs normal | 0-16 | 0% | 大差（下限のみ・下記 F003） |
| normal vs hard | 1-15 | 6% | **≈ 470**（CI 広い） |
| hard vs max | 1-15 | 6% | ≈ 470（max は 500k ノード計測＝過小・下記 F001） |
| easy vs hard | 0-16 | 0% | 大差（下限のみ） |

**結論（控えめに・過剰主張しない）**: プリセットは **順序どおり分離**（upset なし・各段で下位が大差負け）し、**難度選択が意味を持つ**ことは確認できた。一方で **正確な Elo 値は tight には確定していない**（下記 Codex ゲート② の 3 指摘）。よって表示 elo（800/1400/1900/2800）は「実測と矛盾しない」水準に留め、**値は変更しない**（過小データに恣意的な新値を当てない）。絶対 Elo は将棋で基準が曖昧（ウォーズ/81dojo/floodgate で桁違い）なので「~」目安表記は維持。

**Codex ゲート② 方法論指摘（medium 3・全採用）と扱い**:
- **F001（node vs movetime）**: ハーネスは `go nodes`、アプリは `go movetime`。ただし easy/normal/hard は 40k-280k ノード（≈40-280ms）で NodesLimit が movetime(400-1000ms)より先に効く＝node 計測はアプリに忠実。max のみアプリ（無制限/1500ms）と乖離＝過小測定。→ ハーネスに `MEASURE_MOVETIME=1`（movetime 完全再現）モードを追加。
- **F002（初期局面のみ）**: 全局 hirate 開始で局面偏りあり。→ ハーネスに開始局面サンプリング（`OPENINGS` 巡回）を追加。現行の掲載結果は hirate 主体である旨を明記。
- **F003（飽和からの過剰結論）**: 0/16 の Elo 下限は片側95%で約 **275**（500 でない）。→ 「表示値と整合＝検証済み」の主張を撤回し、上記のとおり「順序と分離は確認、正確な Elo は未確定、値は矛盾しないので不変」に downgrade。

**厳密化する場合の再測定**: `MEASURE_MOVETIME=1 node scripts/measure-shogi-elo.mjs 100`（movetime 忠実 + 開始局面多様化 + 局数増）で非飽和の勝率と信頼区間を得る。プリセットの skill/nodes を変えたら回帰確認にも使う（ハーネスをコミット済み）。
