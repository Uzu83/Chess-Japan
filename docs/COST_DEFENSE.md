...
## ⚠️ 公開前にやること（Codex レビュー＝**保留(HOLD)** 判定の3条件）

2026-06-29〜30 の Codex 異モデルレビューで、**実キー接続して公開する前に必ず潰すべき**ブロッカーが3件確定した。
Claude API 統合そのものは「正しい」と確認済み。**この3つを満たすまで本番キー接続・一般公開をしない。**

### 1. 【HIGH】fail-open を本番では fail-closed にする

現状 `rateCheck` は DB エラー時に `return true`（fail-open）。DB 一時障害で正当ユーザーを巻き込まない狙いだが、
**ホスト環境では「レート制限が壊れている＝課金無防備」を意味する**。望ましい挙動:

- 本番（`IS_HOSTED`）で `STORE_READY` が false → **503**（共有ストア無しでLLMを叩かせない）。
- 本番で `rate_check` RPC が失敗 → **fail-closed（503/429）**。ローカル/dev は従来どおり素通しでよい。

該当: `explain/index.ts` の `rateCheck`（126–144行付近のコメントに「要再評価」と明記済み）。
